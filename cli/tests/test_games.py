...
def test_can_create_game(runner, app, server_url, test_game_response):
    ...
    result = runner.invoke(app, [
        ...
        "--logo-url", "https://image-game.com/game.png"
    ])
...
def test_error_on_getting_non_existent_game(runner, app, server_url):
    ...
    httpretty.register_uri(
        method="GET",
        uri=f"{server_url}/api/games/{non_existent_user_id}",
        status=404
    )
...
def test_can_get_game(runner, app, server_url, test_game_response):
    ...
    result = runner.invoke(app, ["games", "get", "--id", test_game_response['game']['id']])
    assert result.exit_code == 0, result.exception
...
def test_can_update_game_publisher(runner, app, server_url, test_game_response):
    ...
    result = runner.invoke(app, ["games", "update", "--id", test_game_response["game"]["id"], "--publisher", new_publisher])
    assert result.exit_code == 0, result.stdout
...
def test_can_update_game_title(runner, app, server_url, test_game_response):
    ...
    result = runner.invoke(app, ["games", "update", "--id", test_game_response["game"]["id"], "--title", new_title])
    assert result.exit_code == 0, result.exception
...
def test_can_delete_game(runner, app, server_url, test_game_response):
    ...
    result = runner.invoke(app, ["games", "delete", "--id", test_game_response["game"]["id"]], input="y")
