...
def test_error_on_getting_non_existent_user(runner, app, server_url):
    ...
    httpretty.register_uri(
        method="GET", uri=f"{server_url}/api/users/{non_existent_user_id}", status=404
    )
...
def test_can_list_users(runner, app, server_url, test_user_response):
    users = {"count": 1, "users": [test_user_response["user"]]}
    httpretty.register_uri(
        method="GET", uri=f"{server_url}/api/users", body=json.dumps(users), status=200
    )
...
def test_can_get_user(runner, app, server_url, test_user_response):
    ...
    result = runner.invoke(
        app, ["users", "get", "--id", test_user_response["user"]["id"]]
    )
    assert result.exit_code == 0
...
def test_can_update_user_age(runner, app, server_url, test_user_response):
    ...
    result = runner.invoke(
        app,
        ["users", "update", "--id", test_user_response["user"]["id"], "--age", new_age],
    )
    assert result.exit_code == 0
...
def test_can_delete_user(runner, app, server_url, test_user_response):
    ...
    result = runner.invoke(
        app, ["users", "delete", "--id", test_user_response["user"]["id"]], input="y"
    )
